...
@interface XWHWorkFlowSearchView () <UITextFieldDelegate>
...
@implementation XWHWorkFlowSearchView

- (void)awakeFromNib
{
    UIImage *image = [UIImage imageNamed:@"bulletin_kindBg"];
    image = [image stretchableImageWithLeftCapWidth:floorf(image.size.width/2.0) topCapHeight:0];
    self.criteriaImgView.image = image;
